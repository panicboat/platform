...
| Addon / Resource | 配置 | 役割 |
|---|---|---|
| Karpenter controller | `karpenter` namespace | Pod 需要に応じて EC2 instance を動的 provision（system-components NodePool が起動する m8g/c8g/r8g 系 Graviton 4 instance）。`karpenter:karpenter` ServiceAccount は **Pod Identity Association** 経由で IAM role assume |
...
| NodePool `system-components` | (cluster-scoped) | requirements: arm64 + on-demand + Graviton 4 (gen Gt 7) + size medium-4xlarge。`WhenUnderutilized` consolidation + 720h expireAfter |
