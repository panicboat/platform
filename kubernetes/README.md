...
| Addon / Resource | 配置 | 役割 |
|---|---|---|
| Karpenter controller | `karpenter` namespace | Pod 需要に応じて EC2 instance を動的 provision（system-components NodePool が起動する m6g/m7g/m8g 系 Graviton instance、category `m` only / generation Gt 5）。`karpenter:karpenter` ServiceAccount は **Pod Identity Association** 経由で IAM role assume |
...
| NodePool `system-components` | (cluster-scoped) | requirements: arm64 + spot + on-demand + general-purpose (m only、gen Gt 5 = m6g/m7g/m8g+) + size medium-4xlarge。`WhenEmptyOrUnderutilized` consolidation + 720h expireAfter |
...
| `kubernetes/components/mimir/production/` | `monitoring` namespace | chart `grafana/mimir-distributed` v6.0.6 (Microservices mode)。8 enabled component (gateway / distributor / ingester / querier / query-frontend / store-gateway / compactor / chunks-cache) + 5 disabled component (alertmanager / ruler / query-scheduler / overrides-exporter / smoke-test) |
...
| Mimir Pod Identity | `monitoring:mimir` SA → `eks-production-mimir` IAM role | Mimir pod が S3 backend へ access するため。aws/eks stack で Pod Identity Association を provision |
...
# Karpenter
kubectl get deployment -n karpenter karpenter             # READY 2/2、controller_host MNG 上で稼働
...
| Pending pod があるのに Karpenter が node を起動しない | NodePool の requirements にマッチする instance type が region で出ない (Graviton 4 capacity 不足等) or `limits.cpu` に達している。`kubectl describe pod <pending-pod>` の events で Karpenter の判断ログを確認、必要なら一時的に `instance-generation: Gt 4` で gen 5+ も許可、もしくは `instance-category` に `c`, `r` を追加 |
