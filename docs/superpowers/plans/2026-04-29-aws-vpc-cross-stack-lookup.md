...
`include "root"` は意図的に持たない（state を必要としない動作確認のため）。

- [ ] **Step 6: Run terragrunt init / validate (AWS auth 不要)**

Run: `cd aws/_test-consumer/envs/production && terragrunt init`
Expected:
- `.terragrunt-cache/<hash1>/<hash2>/` に `main.tf` などがコピーされる
- 同階層に `vpc/lookup/` ディレクトリがコピーされている（`include_in_copy` の効果）
- `Terraform has been successfully initialized!`

確認: `find .terragrunt-cache -type d -name lookup` が 1 件以上ヒットする。
...
将来の consumer（EKS / ECS / RDS / ALB）の PR では、各 stack の `terragrunt.hcl` に `terraform { source = "../../modules"; include_in_copy = ["../../vpc/lookup"] }` を設定し、`modules/main.tf` で `module "vpc" { source = "../../vpc/lookup"; environment = var.environment }` と参照する。
