...
Terragrunt は consumer の `terragrunt.hcl` 評価時に `terraform.source` で指定したディレクトリをキャッシュ（`.terragrunt-cache/<hash1>/<hash2>/`）にコピーし、その内部で `terraform` を実行する。consumer の `main.tf` が `module "vpc" { source = "../../vpc/lookup" }` と書いても、相対解決はキャッシュ内基準になるため lookup には到達できない。

これを解決するため、各 consumer の `terragrunt.hcl` で Terragrunt の `include_in_copy` を使い、キャッシュ作成時に lookup ディレクトリも同梱する。

```hcl
# aws/<consumer>/envs/<env>/terragrunt.hcl の terraform ブロック
terraform {
  source          = "../../modules"
  include_in_copy = ["../../vpc/lookup"]
}
```

`include_in_copy` のパスは `terraform.source` 解決後の dir からの相対指定（= `aws/<consumer>/modules/` 起点で `../../vpc/lookup` → `aws/vpc/lookup`）。これによりキャッシュ内に `vpc/lookup/` が同じ相対構造で配置され、`module "vpc" { source = "../../vpc/lookup" }` が解決される。
...
terragrunt init
terragrunt validate
terragrunt plan   # AWS 認証が通っていれば lookup の data source が解決され、output debug の値が表示される
```

期待: キャッシュに `vpc/lookup/` が同梱され、`module.vpc.*` が解決される。確認後 `aws/_test-consumer/` をディレクトリごと削除し、本 PR の最終 commit には残さない。
...
- **consumer 側 `terragrunt.hcl` に `include_in_copy` 行が必要**: 各 consumer が lookup を参照する際に `include_in_copy = ["../../vpc/lookup"]` を 1 行追加する必要がある。git source 化すれば不要になるが、network 依存と silent な version 流入を避けるため本 spec ではローカル参照を採用
