...
- 調査の結果、Loki は OTLP ingest 時に `k8s.deployment.name` / `k8s.statefulset.name` / `k8s.daemonset.name` / `k8s.cronjob.name` / `k8s.job.name` を index label へ昇格するデフォルト設定を既に持っている（VERIFIED、Grafana 公式ドキュメント）。ギャップは OTel Collector の `k8sattributes` processor が Deployment 以外の owner kind を extract していない点にある
- `k8sattributes` processor の `extract.metadata` 未指定時のデフォルトは6項目のみ: `k8s.namespace.name` / `k8s.pod.name` / `k8s.pod.uid` / `k8s.pod.start_time` / `k8s.deployment.name` / `k8s.node.name`（VERIFIED、upstream README）
- 現行 ClusterRole (`helm template` で実際に render して確認、VERIFIED) は `pods` / `namespaces` (core) と `replicasets` (apps/extensions) の `get,watch,list` のみ
...
- k8sattributes processor が `k8s.deployment.name` / `k8s.statefulset.name` / `k8s.daemonset.name` / `k8s.job.name` / `k8s.cronjob.name` を個別に extract しても、pod ごとに **どれか1つしか値が入らない**ため、Grafana 変数の `label_values()` は単一 label しか引けず、そのままでは統一的な `$workload` filter を組めない
...
Pod (Deployment/StatefulSet/DaemonSet/CronJob 配下)
  ...
  │   extract.metadata に k8s.statefulset.name / k8s.daemonset.name /
  │   k8s.job.name / k8s.cronjob.name を追加（既存の6項目 + 4項目）
...
### 変更点 1: `k8sattributes` processor の extract.metadata 拡張

`kubernetes/components/opentelemetry-collector/production/values.yaml.gotmpl` の `config.processors` に `k8sattributes` を明示追加し、default の6項目 + 以下4項目を extract する:
...
### 変更点 2: `transform/workload` processor の新規追加
...
### 変更点 4: Loki 側で `workload` を index label に昇格
...
| リスク / オープン課題 | 影響 | 対応方針 |
|---|---|---|
| `k8s.cronjob.name` が k8sattributes processor で追加 API lookup なしに取れるか未確認 | CronJob 配下の pod の workload が不安定 (Job 名のまま) になる可能性 | 実装時に実 pod で検証。取れない場合は正規表現ベースの OTTL fallback に切り替え、必要なら ClusterRole に `batch/v1 jobs` の get,watch,list を追加 |
