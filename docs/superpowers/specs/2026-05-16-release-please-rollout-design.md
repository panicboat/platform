...
- 値は「導入 PR のベース main HEAD SHA」(= 導入 PR 用のブランチを切った時点の `origin/main` の SHA)
- release-please は `bootstrap-sha` より新しいコミットを CHANGELOG の対象として扱う
- 結果として導入 PR (release.yml 追加のコミット) は初回 release の CHANGELOG に含まれる。「Introduce release-please workflow」相当の chore コミットが Initial release ノートに残る形になり、許容できる
- 導入 PR をベースから rebase し直した場合は bootstrap-sha も再取得して更新する必要がある
...
1. 導入 PR 用のブランチを `origin/main` から切る
2. `git rev-parse origin/main` で取得した SHA を `bootstrap-sha` に埋め込んだ `release.yml` (および monorepo は config/manifest) を作成
3. PR を作成・マージ
4. マージを契機に release-please PR が自動生成されることを確認
