...
### G2: Grafana Mimir chart (Read-Write mode) を deploy

Mimir Read-Write mode で long-term metrics storage を提供する。Prometheus が remote write 経由で Mimir distributor に metrics を送り、Mimir ingester が S3 (Sub-project 1 で provision) に flush。長期データは Mimir querier / store-gateway 経由で Grafana から query 可能。

Read-Write mode の構成:
- **Write-path** (1 deployment): distributor + ingester
- **Read-path** (1 deployment): querier + store-gateway + query-frontend + compactor
...
- Mimir Microservices mode への移行 — cluster 規模拡大 (10M+ active series) 時の Future Spec
...
- **grafana/mimir-distributed** (Grafana Labs 公式): Mimir の各 component を deploy する chart。Read-Write mode で 2 deployment (Write-path + Read-path) を起動。
...
### Decision 2: Mimir deployment mode = Read-Write

Read-Write mode を採用する。Write-path (distributor + ingester) と Read-path (querier + store-gateway + query-frontend + compactor) を 2 deployment に分離した production-recommended な構成。

**根拠**:

- Grafana Mimir 公式 docs で **production-recommended for small-medium cluster**
- panicboat の規模 (production EKS、Karpenter system-components NodePool 2-4 nodes、active series ~100K-1M 想定、1 tenant) に最適
- Microservices mode (各 component 別 deployment) は overkill (10M+ active series / multi-tenant 規模で必要)
- Monolithic mode (1 binary 全 features) は公式に "not recommended for production"

将来 cluster 規模拡大時の Microservices mode への移行は chart values の `mode` change のみで可能 (StatefulSet name conflict には注意)。
...
### Decision 9: PVC sizing

各 component の persistent storage 要件:

| Component | 用途 | size | StorageClass |
|---|---|---|---|
| Prometheus | TSDB (retention 24h、long-term は Mimir に remote write) | 20 GiB | gp3 (EKS 既定) |
| Alertmanager | alert state | 2 GiB | gp3 |
| Grafana | dashboard / user state (DB は SQLite local) | 5 GiB | gp3 |
| Mimir ingester | WAL buffer + recent metrics | 10 GiB | gp3 |
| Mimir store-gateway | S3 cache (chunks + index) | 20 GiB | gp3 |
| (オプション) Mimir compactor | temporary local storage | 10 GiB | gp3 |

合計: 57 GiB (compactor PVC 不要の場合) / 67 GiB (compactor PVC 含む)。EBS gp3 月額 ~$0.08/GiB なので **~$5-6/month**。

将来 active series が増えた場合、observation で sizing 見直し (=resource adjustments の plan で対応)。

### Decision 10: Resource requests/limits = production-grade
...
| Mimir Write-path (distributor + ingester) | 500m | 1Gi | 2 | 4Gi |
| Mimir Read-path (querier + store-gateway + query-frontend + compactor) | 500m | 1Gi | 2 | 4Gi |

panicboat の Karpenter NodePool 上 m6g.medium (1 vCPU / 4 GiB) で 1 Pod は乗らない可能性あり (= Mimir Write-path の `requests CPU 500m` は m6g.medium 1 vCPU の 50%)。Karpenter は requests を見て m6g.large (2 vCPU / 8 GiB) 等の上位サイズで provision する想定。
...
| K8s (helmfile) | `kubernetes/components/mimir/production/helmfile.yaml` | grafana/mimir-distributed chart install (Read-Write mode) |
| K8s (helmfile) | `kubernetes/components/mimir/production/values.yaml.gotmpl` | Mimir Read-Write mode + S3 backend (Sub-project 1 outputs) + Pod Identity (`serviceAccount.name=mimir`) + resources / PVC |
...
2. **Flux reconcile (K8s 側)**:
   ...
   - grafana/mimir-distributed chart install (Mimir Read-Write mode、Pod Identity Association `monitoring:mimir` で S3 access)
...
- **Mimir Read-Write mode の HA 化**: 各 deployment が default で 1 replica (= chart values で `replicas: 1`)、ingester の AZ-spread はしない (本 phase は単 AZ で start、Phase 5 / 6 で multi-AZ 検討)
...
- **Mimir compactor を本 sub-project で deploy**: Mimir Read-Write mode で compactor も Read-path に含まれる。S3 cost 削減のため compaction を有効化 (= Thanos Compactor と同等の役割を Mimir 内で兼務)
