...
**Cost Optimization Hub のリソースは Terraform で管理しない**。`cost_optimization_hub.tf` はその経緯を残す **コメントのみ** のファイルとして配置する。

理由 (provider 側の制約 2 件、いずれも HCL では回避不能):

- **`aws_costoptimizationhub_enrollment_status`**: AWS API が `include_member_accounts` を Read で返さないため refresh で state が null になり、毎回 plan で `+ include_member_accounts = false` / `~ status -> (known after apply)` の 1 件 perpetual diff が発生する。`lifecycle.ignore_changes`（`= all` を含む）でも抑止不可能。Apply は冪等だが、毎回の diff が他の本物の変更を埋もれさせる。
- **`aws_costoptimizationhub_preferences`**: provider が `member_account_discount_visibility` を未指定でもデフォルト値 `"All"` で API に送り、AWS は non-management account に対し `ValidationException: Only management accounts can update member account discount visibility.` で拒否するため一切 apply できない。

運用:

- 初回の enrollment は当初 Terraform で実施 (PR #262 / #264) し、その後 `terragrunt state rm` で state からのみ除去した。AWS 側 enrollment は永続的に Active のまま残る (一度 enroll すれば再 enroll は不要)。
- `savings_estimation_mode` は AWS デフォルト `"AfterDiscounts"` を使う (`aws cost-optimization-hub get-preferences` で確認)。standalone account には enterprise discount 契約が無いため `BeforeDiscounts` と `AfterDiscounts` は同じ値を返す。
- Organization の management account になった時点で provider issue が解消されていれば、リソース管理を再開する。
